# Title: Stage 3 Incidence and Prevalence Forecast by ARIMA Shiny Application, incidence tab
       ...
       layout_sidebar(
         ...
         sidebar = sidebar(width = "25%",
                           ...
                           sliderTextInput(inputId = ns("select_year"),
                                           ...
                                           choices = c(seq(1990,2025)), # sliderTextInput used (for formatting reasons)
                                           selected = "2025",
                                           grid = TRUE,
                                           width = "95%",
...
incidenceServer <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
      ...
      if (input$help > 0)  {
        showModal(modalDialog(
          ...
          HTML("<p style = 'font-size: 150%;'><p><b>Directions</b><br>
                        ...
                        For incidence details, hover mouse over a coloured region. Scroll to zoom in and out of the map.<br><br>
    ...
    # create dataset based on user input for map
    inc_react <- reactive({
      if (input$subset == "dis") {
        inc_react <- inc_dis %>% 
      ...
      inc_react %>% filter(name != "Global")
    })


    # create palette with bins that auto-adjust to full rate range
    inc_rates <- data.frame(rate = c(inc_dis$rate, inc_age$rate, inc_sex$rate))

    bins <- c(0, round(max(inc_rates$rate)/5), round(2*(max(inc_rates$rate)/5)), round(3*(max(inc_rates$rate)/5)), round(4*(max(inc_rates$rate)/5)), round(5*(max(inc_rates$rate)/5)))

    # create legend breaks to add to leaflet below
    legend <- c(paste0("0.0–", as.character(round(max(inc_rates$rate)/5), digits = 1), ".0"),
                paste0(as.character(round(max(inc_rates$rate)/5) + 0.1), "–", as.character(round(2*(max(inc_rates$rate)/5))), ".0"),
                paste0(as.character(round(2*(max(inc_rates$rate))/5) + 0.1), "–", as.character(round(3*(max(inc_rates$rate)/5))), ".0"),
                paste0(as.character(round(3*(max(inc_rates$rate))/5) + 0.1), "–", as.character(round(4*(max(inc_rates$rate)/5))), ".0"),
                paste0(as.character(round(4*(max(inc_rates$rate))/5) + 0.1), "–", as.character(round(5*(max(inc_rates$rate)/5))), ".0")
    )

    # manual specify bin colours
    map_pal <- colorBin(palette = c("#DBFEF4", "#8EFEDC", "#1B9E77", "#016245", "#002B1E"), domain = inc_rates$rate, bins = bins, pretty = FALSE)
    ...
    observeEvent(inc_react(), {
    #     
    #     forecast_c_str <- ifelse(country_react()$forecast == 1, "forecasted", "observed")
        label = paste0("<span style = 'font-size: 125%;'><b>Region</b>: ", unique(inc_react()$name_format),
                       "<br><b>Disease type</b>: ", unique(inc_react()$disease_type),
                       "<br><b>Age group</b>: ", unique(inc_react()$agegrp),
                       "<br><b>Sex</b>: ", unique(inc_react()$sex),
                       "<br><b>Year</b>: ", unique(inc_react()$year),
                       "<br><b>Incidence rate</b>: ", paste0(sprintf("%.1f", round(inc_react()$rate, 1)), " (", sprintf("%.1f", round(inc_react()$lb, 1)), ", ", sprintf("%.1f", round(inc_react()$ub, 1)), ")"),
                       "<br><b>Rate type</b>: ", unique(inc_react()$forecast)) %>% lapply(htmltools::HTML)

        leafletProxy("map", data = inc_react()) %>%
          clearShapes() %>%
          addPolygons(fillColor = ~map_pal(inc_react()$rate),
                      color = "#363538",
                      weight = 2,
                      stroke = TRUE,
                      layerId = ~inc_react()$name, # layers have to have unique ids for click_data to work
                      label = ~label,
                      fillOpacity = 0.55,
          ...
          addControl(layerId = "year_control", 
                     paste0(HTML("<span style = 'font-size:175%; color: #363538;'>", unique(inc_react()$year), "</span>")), 
                     position = "topleft") 
        # add a highlight to regions that are selected for line plot
          # clearGroup("selected") %>%
          # addPolygons(data = geo %>% filter(name %in% RV$selected),
          #             group = "selected",
          #             fillColor = NULL,
          #             color = "yellow",
          #             weight = 2)
    ...
    global_react <- reactive({
      data <- switch(input$subset,
                     "dis" = inc_dis %>% filter(disease_type == input$select_dis),
                     "age" = inc_age %>% filter(agegrp == input$select_age),
                     "sex" = inc_sex %>% filter(sex == input$select_sex))
      data %>% 
    ...
    output$plot <- renderPlotly({
        plot_ly() %>%
          ...
          add_trace(data = global_react(),
                    ...
                    name = "Overall") %>%
          add_annotations(x = input$select_year-1, y = 0.9*y_max(),
                          text = "Map Year",
                          textangle = -90,
                          font = list(color = "grey",
                                      size = 10),
                          showarrow = FALSE) %>% 
          layout(title= "Incidence Temporal Trends",
    ...
    observeEvent(input$map_shape_click, {
      ...
      # tally number of clicks on each region
      # click <- input$map_shape_click
      # RV$click_data <- c(RV$click_data, click$id)
      # click_freq <- as.data.frame(table(unlist(RV$click_data))) %>%
      #   filter(Freq %% 2 == 1) # remove even number of clicks
      ...
      output$plot <- renderPlotly({
        plot_ly() %>%
          ...
          add_trace(data = plot_react() %>% filter(forecast == "Observed"),
                    ...
                    name = ~name,
                    hoverinfo = "none") %>%
          add_trace(data = plot_react(), # %>% filter(forecast == "Forecasted" | (forecast == "Observed" & year == max(year[forecast == "Observed"], na.rm = TRUE))),
                    x=~year, y = ~rate,
                    type = "scatter", mode = "lines",
    ...
    output$download <- downloadHandler(
      filename = function(){
        paste("Stage3_incidence_data_", Sys.Date(), ".csv", sep = "")
      },
      content = function(file){
        write.csv(inc_dl, file, row.names = FALSE)
      }
    )
